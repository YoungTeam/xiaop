  ...
  ;判断是否开机运行程序
   ${If} $CheckAutoStart_State == 1
	;MessageBox MB_OK '选中'
     ...
     WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "SOFTWARE\Microsoft\Windows\CurrentVersion\Run" "Pandora" "$INSTDIR\Pandora.exe"
   ${EndIf}
...
Section Uninstall
  ...
  Delete "$INSTDIR\Pandora.exe"
  Delete "$INSTDIR\PSkin.dll"
