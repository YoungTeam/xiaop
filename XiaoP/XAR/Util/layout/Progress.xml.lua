...
function Progress_OnInitControl(self)
	...
	local attr = self:GetAttribute()
	--临时去掉移动光
	attr.ShowMovingLight = false
	
	
	if attr.ShowMovingLight then
		local templateMananger = XLGetObject("Xunlei.UIEngine.TemplateManager")
		local poscartoon = templateMananger:GetTemplate("Progress.Light.Animation","AnimationTemplate")
		local instance = poscartoon:CreateInstance()
		if instance then
			local lightbkg = self:GetControlObject("LightBkg")
			local leftbkg, topbkg, rightbkg, bottombkg = lightbkg:GetObjPos()
			local bkgwidth, bkgheight = rightbkg - leftbkg, bottombkg - topbkg
			local lightwidth = 126 * (bkgheight / 23)
			local posoffect = lightwidth / 4
			instance:SetKeyFrameRect(-posoffect, 0, -posoffect + lightwidth, bottombkg, bkgwidth - lightwidth + posoffect, 0, bkgwidth + posoffect, bottombkg)
			instance:BindLayoutObj(light)
		...
		light:SetChildrenVisible(false)
	end
