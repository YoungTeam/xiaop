...
function SetState(self, state)
    ...
    if attr.NowState ~= state then
        ...
        elseif attr.NowState == 3 then
			if attr.SelectBkgID ~= "" then
				bkg:SetTextureID(attr.SelectBkgID)
				edit:SetTextColorID(attr.SelectTextColor)
			end
		end
    end
end

function SetIsPassword(self,isPassword)
	local edit = self:GetControlObject("newedit.edit")
	edit:SetIsPassword(isPassword)
end

function SetTextColor(self,color)

	local edit = self:GetControlObject("newedit.edit")
	edit:SetTextColorID(color)
	
	local attr = self:GetAttribute()
	attr.NormalTextColor = color
	attr.HoverTextColor = color
	attr.DisableTextColor = color
	attr.SelectTextColor = color
end
...
function Paste_OnInit( self )
	...
	if not appClass:IsClipboardTextFormatAvailable() then
		self:SetEnable( false )
	end
end
