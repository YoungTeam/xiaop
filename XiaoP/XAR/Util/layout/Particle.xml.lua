...
function Env_OnInitControl(env)
	...
	local updater = XLGetObject("Xunlei.UIEngine.AnimationFactory"):CreateAnimation("Thunder.Particle.Updater")
	updater:Bind(env)
...
function Env_RandomParticle(env, tbl, times)
	for i=1,times do
		local particle = XLGetObject("Xunlei.UIEngine.ObjectFactory"):CreateUIObject("", "Thunder.Particle")
