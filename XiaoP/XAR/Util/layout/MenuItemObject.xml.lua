...
function OnMouseHover(self)
	...
	local menuObj = self:GetOwnerControl()
	
	menuObj:SetHoverItem(self)
	--XLMessageBox(menuObj:GetID())
...
function OnMenuItemSelect(self, name, id)
    ...
    elseif id == "helpmenu.helpcenter" then
        local shell = XLGetObject("Xunlei.UIEngine.OSShell")
        shell:OpenURL("http://help.xunlei.com/")
    elseif id == "toolmenu.findtask" then
	    ...
	    local mainFrame = hostwndManager:GetHostWnd("Thunder.MainFrame")
	    local maintree = mainFrame:GetBindUIObjectTree()
	    local taskMgrWnd = maintree:GetUIObject("thunder.task_mgr_wnd")
	    local imageinfo = taskMgrWnd:GetControlObject("thunder.taskimageinfo")
