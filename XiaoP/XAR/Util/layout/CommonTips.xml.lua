...
function GetTipRect(self, hostCtrl)
	...
	local waLeft,waTop,waRight,waBottom = appClass:GetWorkArea(1,1)
...
function OnPosChange(self)
	...
	local tipsHostWnd = hostWndManager:GetHostWnd("Thunder.Tips.HostWnd")
...
function OnInitControl(self)
	...
	tipsHostWnd = tipsHostWndTemplate:CreateInstance("Thunder.Tips.HostWnd" .. attr.tipId)
...
function OnDestroy(self)
	local hostWndManager = XLGetObject("Xunlei.UIEngine.HostWndManager")
	local tipsHostWnd = hostWndManager:GetHostWnd("Thunder.Tips.HostWnd" .. self:GetAttribute().tipId)
	if tipsHostWnd then
