...
local function SetDefaultAnimation(self)
    ...
    if attr.IsDefaultButton then
        ...
        if objectTree then
	        ...
	        local ani = aniFactory:CreateAnimation("Thunder.Animation.Button.DefaultChange")
	        ani:BindObj(self)
...
function AddTip(self, x, y)
	...
	local xarFactory = xarManager:GetXARFactory()
        ...
        local tipObj = xarFactory:CreateUIObject("tip","Thunder.Windowless.Tip")
	
	local tipAttr = tipObj:GetAttribute()
	tipAttr.BkgTexture = "texture.mainwnd.tip.bkg"
	tipAttr.TextBeginH = 7
...
local function InitPosition(self)
	...
	local obj = self:GetControlObject("button.icon")
	obj:SetObjPos2(attr.IconLeftPos, attr.IconTopPos, attr.IconWidth, attr.IconHeight)
...
function SetButtonStatus(self,status)
	local attr = self:GetAttribute()
	if status ~= 0 then
