	...
	if needFocus then
		...
		tabbedObj:AttachListener("OnTabbedEvent", true, 
								function(obj, funcName, bIsTabbed)	
									if bIsTabbed then
										XLSetGlobal("Thunder.TabKeySupport.TabbedObj", obj)
									else
										XLSetGlobal("Thunder.TabKeySupport.TabbedObj", "nil")
									end
									if onTabbedStateChangeFunc ~= nil then
...
--tab:9  enter:13 左上右下：37 38 39 40
	...
	if keyChar == 9 then
		...
		XLSetGlobal("Thunder.TabKeySupport.TabbedObj", self)	
		if attr.CurrentShowIndex == nil then
			for i = 1, #tabbedObjArray do
...
function GetTabbedObj(self)
	local tabbedObj = XLGetGlobal("Thunder.TabKeySupport.TabbedObj")
