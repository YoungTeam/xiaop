...
function Header_InsertColumn(self, columnInfo, textColor)
	...
	local newItem = objFactory:CreateUIObject(columnInfo.ID, "Thunder.ListCtrl.HeaderItem")
...
function Item_InitItem(self, itemIndex, columnInfoList)
	...
	for i = 1, columnCount do
		...
		local grid = objFactory:CreateUIObject(column.ID, "Thunder.ListCtrl.ItemGrid")
...
function List_InsertColumn(self, columnID, columnWidth, title, textPos, custom, minWidth, bkgtexture, canSort, sortFunc, headerTextColor)
	local attr = self:GetAttribute()
	...
	local columnInfo = {ID = columnID, Width = columnWidth, Title = title, TextPos = textPos, Custom = custom, MinWidth = minWidth, Bkgtexture = attr.HeaderBkgTexture, CanSort = canSort, SortFunc = sortFunc}
