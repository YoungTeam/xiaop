...
function ShowMenu(self, menunHostWndTemplateID, menuTreeTemplateID, menuObjID, parentHandle, x, y,param)
	...
	--[[if not parentHandle then
		local parent = hostwndManager:GetHostWnd("Thunder.MainFrame")
	...
	local lleft, ltop, lright, lbottom = appClass:GetScreenArea( wleft, wtop )
	local rleft, rtop, rright, rbottom = appClass:GetScreenArea( wright, wbottom )
	local sleft, stop, sright, sbottom
