local function UpdateRes(self, state)
	...
	if textFont and textFont ~= "" then
		local charFormat = textObj:GetInsertCharFormat()
		local fontHeight,fontWidth,ret3,ret4,fontWeight,fontItalic,fontUnderline,fontStrikeOut,ret9,ret10,ret11,ret12,ret12,faceName = XLGetObject("Xunlei.UIEngine.XARManager"):GetFont(textFont):GetInfo()
		charFormat.facename = faceName
		charFormat.wight = fontWeight
		charFormat.italic = fontItalic
		charFormat.underline = fontUnderline
		charFormat.strikeout = fontStrikeOut
		textObj:SetInsertCharFormat(charFormat)
	end
	if textColor and textColor ~= "" then
		local charFormat = textObj:GetInsertCharFormat()
		local clrTextColor = XLGetObject("Xunlei.UIEngine. XARManager"):GetColor(textColor)
		charFormat.textcolor = clrTextColor
		textObj:SetInsertCharFormat(charFormat)
	end
	attr.status = state
end

function GetSelectCharFormat(self)
    local edit = self:GetControlObject("edit")
    return edit:GetSelectCharFormat()
end

function SetSelectCharFormat(self, fmt, bCanUndo)
    local edit = self:GetControlObject("edit")
    return edit:SetSelectCharFormat(fmt, bCanUndo)
end

function SetScrollPos(self, bVertical, pos)
    local edit = self:GetControlObject("edit")
    return edit:SetScrollPos(bVertical, pos)
end 
...
function GetRichText(self)
	local edit = self:GetControlObject("edit")
	return edit:GetRichText()
end

function ClearRichText(self)
	local edit = self:GetControlObject("edit")
	return edit:Clear()
end
...
function SetSelTextColor(self,color)
	local edit = self:GetControlObject("edit")
	XLMessageBox(color)
	return edit:SetSelTextColor(color)
end
...
function GetContextHeight(self)
    local edit = self:GetControlObject("edit")
    return edit:GetContextHeight()
end
...
function SetSelBkgColor(self, color)
    local edit = self:GetControlObject("edit")
    return edit:SetSelBkgColor(color)
end

function SetLinkColor(self, color)
    local edit = self:GetControlObject("edit")
    return edit:SetLinkColor(color)
end
...
function Edit_OnHotKey(self, uChar, uRepeatCount, uFlags)
	local owner = self:GetOwnerControl()
end
...
function RemoveTip(self)
    ...
    local tip = root:GetObject("edit.tip")
...
function AddTip(self, text)
    ...
    local tipObj = xarFactory:CreateUIObject("edit.tip","Thunder.CommonTips")
...
function OnGetObjectText(self, strID,obj)
	local control = self:GetOwnerControl()
	local text, handled = control:FireExtEvent("OnGetObjectText",strID,obj)
...
function GetInsertCharFormat(self)
	local edit = self:GetControlObject("edit")
	
	return edit:GetInsertCharFormat()
end

function SetInsertCharFormat(self, fmt)
	local edit = self:GetControlObject("edit")
	
	return edit:SetInsertCharFormat(fmt)
end

function SetNoCaret(self, noCaret)
	local edit = self:GetControlObject("edit")
	
	return edit:SetNoCaret(noCaret)
end

function GetNoCaret(self)
	local edit = self:GetControlObject("edit")
	
	return edit:GetNoCaret()
end
...
function SetSel(self, startPos, endPos)
	local edit = self:GetControlObject("edit")
