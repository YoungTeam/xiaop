...
function OnLButtonDown(self, x, y,flags)
    ...
    if attr.Type == 0 then
	    ...
	    posAni:SetKeyFrameRect(btnLeft, btnTop, btnRight, btnBottom,x - (attr.BtnWidth / 2), 0 - (attr.BtnHeight - height + 2) / 2, x + (attr.BtnWidth / 2), (attr.BtnHeight + height) / 2)
    else
		...
		posAni:SetKeyFrameRect(btnLeft, btnTop, btnRight, btnBottom,0 - (attr.BtnWidth - width + 2) / 2, y - (attr.BtnHeight / 2), (attr.BtnWidth + width) / 2, y + (attr.BtnHeight / 2))
    end
