...
function GetScreenPos(self, x, y, dontCalc)
	...
	local lleft, ltop, lright, lbottom = appClass:GetWorkArea( x, y )
	local rleft, rtop, rright, rbottom = appClass:GetWorkArea( x+width, y+height )
...
function AnimateShow(self)
	...
	if templateMananger then
		...
		if 1 == 1 then --animationTemplate then
			...
			if aniFactory then
				...
				if attr.AnimationTemplateID == "animation.menu.alpha" then
					...
					ani = aniFactory:CreateAnimation("AlphaChangeAnimation")
					frame:SetObjPos2("0","0","father.width","father.height")
					frame:SetLimitChild( false )
