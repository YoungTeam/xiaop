--local dbgprint = debug.bind(debug.print, "combofocus")
...
function CB__AddItem(self, IconResID, IconWidth, LeftMargin, TopMargin, Text, Custom, Func)
    ...
    local comboitem = objFactory:CreateUIObject("item"..num,"Thunder.ComboItem")
...
function CB__Edit__OnChange(self)
	...
	dbgprint("folder path text changed to ",text)
...
function ExpandList( self )
    ...
    if not attr.ShowList then
        ...
        local objFactory = XLGetObject("Xunlei.UIEngine.ObjectFactory")
        listbox = objFactory:CreateUIObject("listbox","Thunder.ComboListBox")
...
function AddTip(self, text)
    ...
    local tipObj = xarFactory:CreateUIObject("combox.tip","Thunder.CommonTips")
